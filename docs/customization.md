...
插槽上下文容器属性为只读；其中的 table、row、column、cell 和 header 仍是原生对象，可以正常调用它们的方法。原生状态兼容性示例提供切片、所有者、上下文引用、渲染次数和引用清理的观察入口。
...
locale 支持局部覆盖并回退简体中文。需要完整替换时，用 ApexLocale 声明完整对象，同时提供数量格式化与可访问名称函数。Localization 示例提供英文对象以及加载、空态、错误、列面板、菜单、分页和所有配置诊断的可操作入口。业务列名、菜单项和提示内容由业务自己翻译。
...
Preferences 示例演示将旧 unitPrice ID 迁移为 price，随后继续按当前允许列和宽度范围清理。恢复不自动写回；用户下一次改动才按新版本保存。主动重新 load 时会丢弃上次写入的去重缓存，即使其他页面已清空记录，也能再次保存相同布局。
